        ...
        ->schema([
            Select::make('employees_id')
                ...
                ->afterStateUpdated(function (callable $set, callable $get, $state) {
                    ...
                    // Recalculate payroll because employee changed
            ...
            Hidden::make('salary_id')
                ...
                ->afterStateUpdated(function (callable $set, callable $get) {
                    // Recalculate if salary_id changes (e.g., when employee changes)
                    static::recalculatePayroll($set, $get);
                }),
            ...
            TextInput::make('basic_salary_display')
                ...
                DatePicker::make('end_date')
                    ...
                    ->required()
                    ->displayFormat('d F Y')
                    ->disabled() // keep it read-only, since it's based on start date + 14 days
                    ->reactive() // add reactive to update form if start_date changes
                    ->dehydrated(),
...
/**
 * Shared method to recalculate payroll.
 * Called when employee, salary, or date range changes.
 */
protected static function recalculatePayroll(callable $set, callable $get): void
{
...

    {
        return $table
            ...
            ->columns([
                Tables\Columns\TextColumn::make('employees_id')->label('Employee ID')->sortable(),
                Tables\Columns\TextColumn::make('salary_id')->label('Salary ID')->sortable(),
                Tables\Columns\TextColumn::make('start_date')->date()->sortable(),
                Tables\Columns\TextColumn::make('end_date')->date()->sortable(),
                Tables\Columns\TextColumn::make('gross_salary')->label('Gross Salary')->money('php')->sortable(),
                Tables\Columns\TextColumn::make('total_deductions')->label('Total Deductions')->money('php')->sortable(),
                Tables\Columns\TextColumn::make('net_salary')->label('Net Salary')->money('php')->sortable(),
